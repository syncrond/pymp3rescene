...
from resample.main import get_file_type, FileType, sample_class_factory

class SRS:
	def __init__(self, filename, binary=None):
		if not os.path.isfile(filename):
			return AttributeError("srs must be a file")
		...
		self.sample = sample_class_factory(get_file_type(self.filename))
